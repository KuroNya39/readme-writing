...
- **语气平实**：不夸张、不堆感叹号。
- **列表动宾化**：功能 / 日常操作条目用「动词短语」加粗领起 + 冒号接说明（**提取候选人信息**：…）；同一动作全文同一动词，不换说法。
- **易错点就地提醒**：读者极可能做错的那一步内给一句 ⚠️（如「勿直接粘贴原始 JD」），不等进 FAQ。
- **表格后收尾**：表格后如需一句解释，单独起一行用引用块 `>`（如「> 上表为……，默认……」）。
...
- [ ] 可扫读：结论在前、一段一主题、无废话
- [ ] 措辞全文统一
- [ ] 功能 / 步骤列表条目以「动词短语」加粗领起，扫加粗词即懂该项做什么
